...
- (void)TPKeyboardAvoiding_keyboardWillShow:(NSNotification*)notification {
    CGRect rect = [self convertRect:[[[notification userInfo] objectForKey:_UIKeyboardFrameEndUserInfoKey] CGRectValue] fromView:nil];
    
    TPKeyboardAvoidingState *state = self.keyboardAvoidingState;
    
    if ( state.keyboardVisible && state.keyboardRect.size.height == rect.size.height) {
        return;
    }
        
    UIView *firstResponder = [self TPKeyboardAvoiding_findFirstResponderBeneathView:self];
    
    state.keyboardRect = rect;
